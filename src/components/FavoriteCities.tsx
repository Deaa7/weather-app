...
export default function FavoriteCities() {
  let { favorites, removeFavorite } = useFavorite();
    ...
    <>
      ...
      <ScrollArea className="w-full pb-4">
              <div className="flex gap-4 ">
                  {
                      favorites.map((e  ) => {
                          return <FavoriteCityTablet key={e.id}  {...e} onRemove={ ()=>removeFavorite.mutate(e.id) }  />
                      } )
                  }
        </div>
      </ScrollArea>
...
function FavoriteCityTablet({
  ...
  lon,
  onRemove,
}: FavoriteCityTabletType) {
  let nav = useNavigate();
  const { data: weather, isLoading } = useWeatherQuery({ lat, lon });

  return (
    <div
      onClick={() => nav(`/city/${name}?lat=${lat}&lon=${lon}`)}
      role="button"
      tabIndex={0}
     className="relative flex min-w-[250px] cursor-pointer items-center gap-3 rounded-lg border bg-card 
      ...
      >
          <Button variant={'ghost'} size={'icon'} className="absolute right-1 top-1 h-6 w-6 rounded-full
           p-0 hover:text-destructive group-hover:opacity-100"
              onClick={e => {
                  e.stopPropagation();
                  onRemove(id);
